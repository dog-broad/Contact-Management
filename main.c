...
void Delete_Contact(GtkWidget* gwt, gpointer data) 
{
    ...
    {
        gtk_label_set_text(GTK_LABEL(ErrorMsg),"You didn't chose a contact to delete in the ComboBox.");
        return;
    }
...
void Show_Contact(GtkButton* btn, gpointer data)
{
    ...
    GtkWidget* diag_display_conatct = (GtkWidget*) gtk_builder_get_object(UIdata->build,"display_contact");
    ...
    if(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(UIdata->combo),&UIdata->comboIter)==0)
    {
        gtk_label_set_text(GTK_LABEL(EditLog),"No Contact Slected, Display Terminated");
        return;
    }
    ...
    gtk_widget_show_all(diag_display_conatct);

    gint res =gtk_dialog_run(GTK_DIALOG(diag_display_conatct));
    ...
    gtk_widget_hide(diag_display_conatct);
}
...
void Print_Contact(GtkButton *btn, gpointer data) 
{
    ...
    if(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(UIdata->combo),&UIdata->comboIter)==0)
    {
        gtk_label_set_text(GTK_LABEL(EditError),"You didn't chose a contact to print in the ComboBox.");
        return;
    }
    ...
    while(valid)
    {
        ...
        fprintf(fp,"%-15s|%-15s|%-15s|%-15s|%-15s|%-15s|\n", edit_contact.Name, edit_contact.Sur_Name, edit_contact.Address, edit_contact.Contact_Type, edit_contact.email, edit_contact.Number);

        i++;
        valid = gtk_tree_model_iter_next(GTK_TREE_MODEL(UIdata->liststore),&UIdata->iter);
    }
    fscanf(fp,"%-15s%*c%-15s%*c%-15s%*c%-15s%-15s%*c%-15s", &s1,&s2,&s3,&s4,&s5,&s6);
    g_print("%-15s %-15s %-15s %-15s %-15s %-15s",s1,s2,s3,s4,s5,s6);
    fclose(fp);
}